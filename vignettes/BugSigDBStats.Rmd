...
microbeHeatmap(dat.feces, tax.level = "genus")
antag.mat <- microbeHeatmap(dat.feces, tax.level = "genus", antagonistic = TRUE)
microbeHeatmap(dat.feces, tax.level = "genus", antagonistic = TRUE)
```
...
# same direction
lcm <- log10(cooc.mat + 0.1)
col <- circlize::colorRamp2(seq(min(lcm), max(lcm), length = 3),
                            c("blue", "#EEEEEE", "red"))
...
# opposite direction
ht2 <- ComplexHeatmap::Heatmap(log10(antag.mat + 0.1),
                               col = col, 
                               name = "log10 count",
                               cluster_columns = FALSE, 
...
```{r}
nasalsub <- subset(dat, `Body site` == "nasal cavity")
sigsub <- bugsigdbr::getSignatures(nasalsub)
paircomp <- calcPairwiseOverlaps(sigsub)
